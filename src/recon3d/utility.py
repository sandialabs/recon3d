...
def compare_files(
    ...
    """
    Compare two text files line-by-line, with options to:
     - drop blank lines
     - drop comment lines (starting with '#')
     - trim all leading/trailing whitespace on each line
     - drop any line containing one of a set of ignore_words

    Returns True if, after all those filters, the files match exactly.
    """
    ...
    def preprocess(lines: list[str]) -> list[str]:
        ...
        for line in lines:
            ...
            if strip_whitespace:
                line = line.strip()
            # if any ignore word appears, drop this line too
    ...
    lines1 = preprocess(lines1)
    lines2 = preprocess(lines2)

    if len(lines1) != len(lines2):
        print(f"Different line counts after filtering: {len(lines1)} vs {len(lines2)}")
        return False

    for i, (l1, l2) in enumerate(zip(lines1, lines2)):
        if l1 != l2:
            print(f"Difference at logical line {i}:")
            print(f"  file1: {l1!r}")
            print(f"  file2: {l2!r}")
            return False
